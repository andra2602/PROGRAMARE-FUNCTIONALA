data Expr = Const Int -- integer constant
          ...
          | Expr :*: Expr -- multiplication
           deriving Eq -- ca  sa nu mai scriem de mana egalitatea de mana ==
-- !!!!!!!!operatorii infix,adica intre operatori mereu trebuie pusi intre : adica :op:    !!!!!!!!
...
-- Empty si Bnode sunt constructorii adica functiile de constructie
-- 
...
values (BNode left _ (Just elem) right) = values left ++ [elem] ++ values right
...
toList Empty = []
toList (BNode left treeKey Nothing right) = toList left ++ toList right
toList (BNode left treeKey (Just val) right) = toList left ++ [(treeKey, val)] ++ toList right
...
printTree Empty = ""
printTree (BNode Empty treeKey val Empty) = show treeKey
printTree (BNode Empty treeKey val right) =  show treeKey  ++ "(" ++ (printTree right) ++ ")"
printTree (BNode left treeKey val Empty) = "(" ++ (printTree left) ++ ")" ++ show treeKey   
printTree (BNode left treeKey val right) = "(" ++ (printTree left) ++ ")" ++ show treeKey  ++ "(" ++ (printTree right) ++ ")"
